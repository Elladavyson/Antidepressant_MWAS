...
phenotype <- read.table('edavyson/Antidep_methylation/antidep_phenotypes/resid_phenos/residualised_selfrep_pheno3_nocolnames.pheno', header = F)
...
qcovs <- read_table('edavyson/Antidep_methylation/OSCA_data/cov_files_30_06/qcovs_11_07_GRMFID.txt') %>% as.data.frame()
covs <- read_table('edavyson/Antidep_methylation/OSCA_data/cov_files_30_06/covs_11_07_GRMFID.txt') %>% as.data.frame()
...
write.table(pheno_residuals, 'edavyson/Antidep_methylation/MRS/selfrep_cov_residuals.txt', row.names = F, quote = F)
