...
log_path='scratch/s2112198/GS_train_DNAm_antidep_450K.log'
...
#DNAm <- read_table('scratch/s2112198/test_probe_standard') %>% as.data.frame() # subset of probes for trouble shooting 
DNAm <- read_table('scratch/s2112198/selfrep_stand_450K_overlap') %>% as.data.frame() # the whole data object
...
resid_pheno <- read.table('scratch/s2112198/selfrep_cov_residuals.txt', header = T)
...
write.table(out, file='scratch/s2112198/big_lasso_450K_selfrep.txt', quote=F, row.names=F, col.names=F)
